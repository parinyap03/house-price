...
import com.example.demoapp.Util.FirestoreRepository
import com.example.demoapp.Util.GlobalBox
import com.example.demoapp.Util.HouseItem
...
//ดึงข้อมูลจากระบบ Online ซึ่งจะเป็นlayerที่ใช้วาดบ้านในแมพ
...
//        fetchDataFromDatabase()
    ...
    //ซูมให้เห็นครอบคลุมทุกจุดทุกบ้าน
    private fun zoomToAllExtent() {
        //สร้างลิสเก็บจุดที่มีในแมพแล้วลูปเพิ่มจุดเข้าไป
        ...
        //GeometryEngine ใช้สร้างวงกลมสีเขียวๆ นอกจากนี้ยังคำนวณค่าต่างๆได้
        var mCompleteExtent: Envelope = GeometryEngine.combineExtents(listOfPoint); //คำนวณว่าควรแสดงแมพยังไงให้เห็นครอบคลุมทุกจุด
        ...
        var mExtentPadding = Envelope(newX1, newY1, newX2, newY2, mCompleteExtent.spatialReference) //สร้างpaddingให้ตำแหน่งต่างๆ ควรทำให้มันซูมออกเพิ่มไม่งั้นจะเห็นไม่หมดหรือไม่สวย
        mapView.setViewpointAsync(Viewpoint(mExtentPadding));
    }
    //เอาข้อมูลเฉพาะที่อยู่ในวงกลม ซึ่่งจะดูก่อนว่ามีเลเยอร์ไหนที่มีข้อมูลตรงในวงกลมนั้นบ้างก็จะเอามาใส่ในfilterdata แล้วก็โชว์บ้านที่อยู่แค่ในวงกลม
...
//add symbol on map
    private fun drawHouseDataOnMap() {
        ...
        for (houseData in houseItemList) {
            ...
            // Point(latitude, longitude, spatial reference) สร้างบ้านที่ได้ดึงมาจากdbมาทำเป็นจุด
            ...
            // add the point graphic to the graphics overlay เลเยอร์ใหม่ที่มีจุดบ้าน
            houseGraphicLayer.graphics.add(pointGraphic)
        }
    }


    //    ใช้วิเคราะห์พื้นที่หรือวัดความยาวของเส้น
    private fun createBufferArea() {
        val userLocationPoin = Point(100.53892960569901, 13.701952685466564, SpatialReferences.getWgs84())
        val geometryBuffer = GeometryEngine.bufferGeodetic(userLocationPoin, 3.0, //3.0คือรัศมีที่เป็นวงกลมสีเขียวสามารถเพิ่มได้
...
//        PointCollection=การลากเส้น
    ...
    private fun addImagePointGraphic() {
        ...
        val pictureMarkerSymbol = PictureMarkerSymbol.createAsync(
            ContextCompat.getDrawable(
                requireContext(),
                R.drawable.marker   //ใช้รูปเป็นสัญลักษณ์แทน
            ) as BitmapDrawable
        ).get()
    ...
    private fun addPointGraphic() {
        // create a point geometry with a location and spatial reference
        // Point(latitude, longitude, spatial reference) ตัวgeomatry
        val point = Point(100.53892960569901, 13.701952685466564, SpatialReferences.getWgs84())
    ...
    private fun addLayerGraphicOverlay() {
        ...
        mapView.graphicsOverlays.add(graphicLayer) //เลเยอร์ที่ซ้อนทับเบสแมพ
        mapView.graphicsOverlays.add(houseGraphicLayer) //house layer
    }
    private fun setupMap() {
        ...
        // set the viewpoint, Viewpoint(latitude, longitude, scale) กำหนดละจิจูด ลองติจูดให้เป็นแผนที่เริ่มต้น
        mapView.setViewpoint(Viewpoint(13.700547, 100.535619, 72000.0))
        mapView.apply {
            // ตอบสนองต่อการคลิกหน้าจอ onSingleTapConfirmedคือกดครั้งเดียว  calloutคือข้อความที่มันจะขึ้นตอนกดบ้าน
            onTouchListener = object : DefaultMapViewOnTouchListener(requireContext(), this) {
                override fun onSingleTapConfirmed(motionEvent: MotionEvent): Boolean {
                    ...
                    val mapPoint = mapView.screenToLocation(screenPoint) //ได้ละ,ลองมาแสดง
                    val identifyGraphics: ListenableFuture<IdentifyGraphicsOverlayResult> =  //identifyGraphicsจะให้ข้อมูลออกมาหลังการจิ้มจอ
                        mapView.identifyGraphicsOverlayAsync( //เมื่อกดลงไปตำแหน่งนึงมันจะหาว่าต้อองเอาข้อมูลเลเยอร์ไหนมาแสดง
                            houseGraphicLayer,
                            screenPoint,
                            10.0,
                            false
                    ...
                    if (results.graphics.size > 0) { //แปลว่าจิ้มจอละเจอข้อมูล
                        val graphicData = results.graphics[0]   //เอาข้อมูลของตำแหน่งที่จิ้มมา
    ...
    //ถ้าผู้ใช้ยุบจอก็จะหยุดเพื่อไม้ให้กินเน็ต
